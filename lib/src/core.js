...
function place(state, placeValues) {
	var immbutablePlaceValues = (0, _immutable.Map)(placeValues);
	var tableSize = state.get('tableSize', TABLE_SIZE);
	if (!(0, _validationJs.validatePlaceValues)(immbutablePlaceValues, tableSize)) {
		return state;
	}
	return state.set('isPlaced', true).set('tableSize', tableSize).set('position', (0, _immutable.Map)({
...
function report(state) {
	...
	return state.update('haveReportet', 0, function (timesReportet) {
