...
function validatePosition(placeValues, tableSize) {
	if (isNotAnInteger(placeValues.get('y')) || isNotAnInteger(placeValues.get('x')) || isNegativeNumber(placeValues) || isCoordinateBiggerThanTableSize(placeValues, tableSize)) {
		return false;
	}
	return true;
}
...
function validateFacing(placeValues) {
	var validFacingValue = (0, _immutable.Map)({
		NORTH: true,
		SOUTH: true,
		EAST: true,
		WEST: true
